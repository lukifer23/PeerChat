...
object PeerDatabaseMigrations {
    ...
    val MIGRATION_2_3 = object : Migration(2, 3) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            // Use transaction for atomicity - if migration fails, database rolls back
            db.beginTransaction()
            try {
                ...
                // Note: FTS tables are virtual and don't need backup/restore
    ...
    val MIGRATION_4_5 = object : Migration(4, 5) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // Add new columns to benchmark_results table for enhanced performance metrics
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN prefillMs INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN decodeMs INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN memoryUsageMB INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN gcCount INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN threadCpuTimeNs INTEGER NOT NULL DEFAULT 0")
        }
    }

    val MIGRATION_5_6 = object : Migration(5, 6) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // Add gpuMode column to benchmark_results table
            db.execSQL("ALTER TABLE benchmark_results ADD COLUMN gpuMode INTEGER NOT NULL DEFAULT 1") // 1 = true (GPU), 0 = false (CPU)
        }
    }
}
